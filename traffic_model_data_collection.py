...
import solution_vehicle_client
import threading
import json
import random
import socket
...
def main(run_time, NUM_SOLUTION_VEHICLES, filename):

    with open(filename, 'wt') as csvfile:
        writer = csv.writer(csvfile)
        # density_range = np.arange(0.25,1,0.25)
        density_range = [x * 0.008 for x in range(1, 63)]
        # density_range = [0.2]
        writer.writerow( ('Trial Number', *density_range) )
        ...
        for run_number in range(0, 1):
            ...
            for density in density_range: 
                ...
                car_count = max(round(ROAD_LENGTH * DENSITY),1)
                if NUM_SOLUTION_VEHICLES > car_count:
                    solution_vehicle_count = car_count
                else:
                    solution_vehicle_count = NUM_SOLUTION_VEHICLES
                interval = round(ROAD_LENGTH / car_count)
                # print('cc: {}, int: {}, dens: {}'.format(car_count, interval, DENSITY))
                for i in range(solution_vehicle_count):
                ...
                results.append(highway_mean_velocity)
                print('Run complete')
                
            print(results)
            writer.writerow( (int(run_number), *results) )
            # for thread in threads:
            #     print('hi {}'.format(thread))
            #     thread.stop()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fn', type=str, default='data.csv')
    args = parser.parse_args()

    run_time = args.run
    NUM_SOLUTION_VEHICLES = args.sv

    main(run_time, NUM_SOLUTION_VEHICLES, args.fn)
